            ...
            for iObjList = 1:length(curSeg.objOrder);
                ...
                if ~keepDataArray(I_object);
                    if dataType == 32
                        ...
                        %NOTE: I think this could be improved because I
                        %think I know somewhere how much to seek ...
                            ...
                            for iString = 1:numValuesAvailable
                                ...
                                fseek(fid,strOffsetArray(iString+1)-strOffsetArray(iString),'cof');
                    ...
                    %NOTE: This is making the assumption that we either
                    %read or don't read a channel. This is not the case in
                    %the _v2 file. Importantly though, we don't need to
                    %use it in the seek above UNLESS using strings.
                    startI = curDataIndex(I_object) + 1;
                    endI   = curDataIndex(I_object) + numValuesAvailable;
                    curDataIndex(I_object) = endI;
...
%==========================================================================
if ~isequal(numValuesToGetActual(keepDataArray),curDataIndex(keepDataArray))
