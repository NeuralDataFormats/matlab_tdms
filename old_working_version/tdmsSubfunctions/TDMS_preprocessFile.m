        ...
        for iNewObject = 1:numNewObjInSeg
            ...
            switch rawDataIndexLength
                ...
                case 2^32-1 %no raw data
                    ...
                    %NOTE: Apparently it is possible to have an object
                    %that is meant to be read, i.e. is present
                    %in the object order, not have raw data
                    %for a given read, and to not set a flag
                    %indicating that a new object list is present
                    if any(objOrder == objIndex)
                        
                       if nSegs == 76
                           keyboard
                       end
                        
                       % neefprintf(2,'Modifying the object order\n');
                       objOrder(objOrder == objIndex) = []; 
                       curObjListCount = curObjListCount - 1;
                    end
                otherwise
                    
                    %This never runs, seems to only be for the 
                    %
                    if ~objectHasRawData(objIndex) && rawDataInfo(objIndex).hadData
                       keyboard 
                    end
                    
                    objectHasRawData(objIndex)    = true;
                    rawDataInfo(objIndex).infoSet = true;
                    rawDataInfo(objIndex).hadData = true;
...
% % % % %         wtf2 = rawDataInfo(objOrder(1:curObjListCount));
% % % % %         
% % % % %         if ~all(objectHasRawData(objOrder(1:curObjListCount)))
% % % % %            wtf_fix = true;
% % % % %            keep_mask = objectHasRawData(objOrder(1:curObjListCount));
% % % % %         else
% % % % %            wtf_fix = false;
% % % % %         end
        ...
        %keyboard
%         if wtf_fix
%             totalBytesPerChunk = sum([wtf2(keep_mask).totalSizeBytes]);
%         else
...
%         end
        ...
        if nChunks ~= floor(nChunks)
            if nChunks < 1 && curObjListCount == 1
                curIndex = objOrder(1);
                nBytes = byteSizeRaw;
                rawDataInfo(curIndex).numberOfValues = nBytes/TDMS_getDataSize(rawDataInfo(curIndex).dataType);
                rawDataInfo(curIndex).totalSizeBytes = byteSizeRaw;
            else
...
%         if wtf_fix
%            objectHasRawData(objOrder(1:curObjListCount)) = true; 
%         end
